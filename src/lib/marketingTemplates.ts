...
// These templates use two families of merge tags, resolved at different
// stages (see lib/mailer.ts):
//   - Recipient tags — {{name}}, {{email}}, and any CSV column — resolved
//     once per recipient.
//   - Brand tags — {{companyName}}, {{website}}, {{contactUrl}},
//     {{address}}, {{accentColor}}, {{unsubscribeText}} — resolved once
//     per send, from whichever domain is selected on the Settings ›
//     Domains page. This is what lets the same template be reused across
//     every configured domain instead of being hardcoded to one brand.
const htmlShell = (bodyInner: string, buttonLabel: string, accent = "{{accentColor}}") => `<!DOCTYPE html>
...
<body><div class="card">
${bodyInner}
<a href="{{contactUrl}}" class="btn">${buttonLabel}</a>
<div class="footer">{{companyName}} · {{address}}<br>
<a href="{{contactUrl}}" style="color:#999">{{unsubscribeText}}</a></div>
</div></body></html>`;
...
<p>{{companyName}} uses blockchain forensics to trace and recover digital assets. Every case starts with a free, no-obligation assessment so you know exactly where you stand before committing to anything.</p>`,
...
{{companyName}} uses blockchain forensics to trace and recover digital assets. Every case starts with a free, no-obligation assessment so you know exactly where you stand before committing to anything.
...
<p>{{companyName}} specialises in exactly this — rebuilding access from partial information, corrupted files, and old backups. No custody of your funds ever changes hands during the process.</p>`,
...
{{companyName}} specialises in exactly this — rebuilding access from partial information, corrupted files, and old backups. No custody of your funds ever changes hands during the process.
...
{{unsubscribeText}}`,
  },
  {
    ...
    subject: "See how our case recovery process works",
    html: htmlShell(
      `<h1>Hi {{name}},</h1>
<p>We know crypto recovery services can be hard to trust — the space has its share of bad actors. Here's our approach: blockchain forensics you can follow step by step, and a free assessment before you commit to anything.</p>
...
We know crypto recovery services can be hard to trust — the space has its share of bad actors. Here's our approach: blockchain forensics you can follow step by step, and a free assessment before you commit to anything.
